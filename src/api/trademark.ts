...
// 获取品牌列表 (前台/分页)
export function getFrontTrademarkListPage(params: {
	pageNo?: number;
	pageSize?: number;
}) {
	return request.get<TrademarkListResponse>("/api/product/trademark/list", {
		params,
	});
}
...
// 获取品牌列表别名（后台）
export const getTrademarkListAlias = (params?: {
	keyword?: string;
	pageNo?: number;
	pageSize?: number;
}) => {
	return request.get<TrademarkListResponse>("/product/trademarkList", {
		params,
	});
};
...
export const updateTrademark = (data: FormData) => {
	return request.put<{ code: number; message: string; logoUrl?: string }>(
		"/product/trademark",
		data,
		{
			headers: { "Content-Type": "multipart/form-data" },
...
export const getTrademarkById = (id: string) => {
	return request.get<{ code: number; message: string; info: Trademark }>(
