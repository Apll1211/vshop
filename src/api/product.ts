...
import type { ProductDetail, Sku, Spu, SpuSaleAttr } from "./types";

// ==================== SPU API ====================

// 获取SPU列表 (前台)
export const getFrontSpuList = (params?: {
	category1Id?: string;
	category2Id?: string;
	category3Id?: string;
	pageNo?: number;
...
// 根据分类获取SPU列表 (后台)
export const getAllSpuListByCategory = (
	categoryId: string,
	params?: { shopId?: string },
) => {
	return request.get<{ code: number; message: string; spuList: Spu[] }>(
		`/product/allSpuList/${categoryId}`,
		{ params },
	);
};
...
export const getSpuDetail = (id: string) => {
	return request.get<{ code: number; message: string; info: Spu }>(
		`/product/spu/${id}`,
	);
};
...
export const updateSpu = (data: FormData) => {
	return request.put<{ code: number; message: string; imageUrls?: string[] }>(
		"/product/spu",
		data,
		{
			headers: { "Content-Type": "multipart/form-data" },
...
export const getSpuSaleAttrList = (spuId: string) => {
	...
	}>("/product/spuSaleAttrList", { params: { spuId } });
};
...
export function getTopProducts(type: string | number, limit: number = 4) {
	return request.get<{ code: number; message: string; skuList: Sku[] }>(
...
// 获取属性列表 (前台)
export function getFrontAttrList(
	category1Id: string,
	category2Id: string,
	category3Id: string,
) {
	return request.get(
		`/api/product/attr/list/${category1Id}/${category2Id}/${category3Id}`,
	);
}
...
export const getAttrByCategoryId = (categoryId: string) => {
	return request.get<{ code: number; data: any[] }>(
		`/product/attr/${categoryId}`,
	);
};
...
export function searchProducts(params: SearchParams) {
	return request.get<{ code: number; data: any[]; total: number }>(
		"/api/product/search",
		{ params },
	);
}
