...
export function uploadAvatar(formData: FormData) {
	return request.post<{ code: number; message: string; avatar: string }>(
		"/api/user/avatar",
		formData,
		{
...
export function getHotTags(limit: number = 10) {
	return request.get<{ code: number; message: string; keywords: string[] }>(
		`/api/user/hotTag/${limit}`,
	);
}
